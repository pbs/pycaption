...
- If it exists: read it as the primary reference alongside the files above
...
- Parity: parity requirement (mark N/A for SCC text format)
...
- If available, read ai_artifacts/specs/scc/standards_summary.md (local only, generated by `/generate-standards-summary`)
- Search web for any missing requirements
