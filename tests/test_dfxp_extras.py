...
from pycaption.dfxp import (DFXP_DEFAULT_REGION, DFXP_DEFAULT_REGION_ID,
                            DFXPReader, LegacyDFXPWriter,
                            SinglePositioningDFXPWriter)
from pycaption.dfxp.base import _create_internal_alignment
from pycaption.geometry import (Alignment, HorizontalAlignmentEnum, Layout,
...
class TestSinglePositioningDFXPWRiter:
    ...
    def test_only_the_custom_region_is_created(
        ...
        internal_alignment = _create_internal_alignment(text_align, display_align)
...
class TestLegacyDFXPWriter:
    def test_default_style_is_written_to_output_file(
        ...
        caption_set = DFXPReader(read_invalid_positioning=True).read(
            sample_dfxp_with_templated_style.format(style_name="foxy_the_squirrel")
        )
