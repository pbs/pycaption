...
TIMING_LINE_PATTERN = re.compile('^(\S+\ \d{2})\|(\S+\ \d{2})\|(\d*)\|(.*)$')
TIMESTAMP_PATTERN = re.compile('^(\d+):(\d{2}):(\d{2})\ (\d{2})')
VOICE_SPAN_PATTERN = re.compile('^\S+\ \d{2}\|\S+\ \d{2}\|\d*\|(.*)$')
...
def microseconds(h, m, s, f):
    ...
    :rtype: int
    """
    # TODO il faut calculer f comme des frames...
    return (int(h) * 3600 + int(m) * 60 + int(s)) * 1000000 + int (int(f)/29.970 * 1000 * 1000)


class TechnoleadsReader(BaseReader):
    ...
    def detect(self, content):
        return 'TECHNOLEADS' in content

    def read(self, content, lang='en-US'):
        if type(content) != six.text_type:
            raise InvalidInputError('The content is not a ISO-8859-1 string.') #was unicode
    ...
    def _parse(self, lines):
        ...
        for i, line in enumerate(lines):
            ...
            if i > 2 :
                ...
                linesText = textwrap.wrap( self._decode(line), 32)
                linesText_len = len(linesText) - 1
                for j, lineText in enumerate(linesText):
                    nodes.append(CaptionNode.create_text(lineText + " "))
                    # if j == len(linesText) :
                    # if (j+1) % 4 == 0 :
                    if len(nodes) == 4 or j == linesText_len:
                        caption = Caption(
    ...
    def _parse_timestamp(self, timestamp):
        ...
        return microseconds(m[0], m[1], m[2], m[3])
    ...
    def _decode(self, s):
        ...
        s = s.strip()
        # Covert voice span
        s = VOICE_SPAN_PATTERN.search(s)
        return s[1]


class TechnoleadsWriter(BaseWriter):
    HEADER = 'Technoleads\n\n'
    global_layout = None
    video_width = None
    video_height = None
