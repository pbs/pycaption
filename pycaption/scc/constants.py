...
POSITIONING_TO_PAC_MAP = _create_position_to_bytes_map(PAC_BYTES_TO_POSITIONING_MAP)
...
CHARACTER_TO_CODE = {character: code for code, character in CHARACTERS.items()}
...
ITALICS_COMMANDS = {
    key: COMMAND_LABELS[key]
    for key in COMMAND_LABELS
    if "italic" in COMMAND_LABELS[key].lower()
}

UNDERLINE_COMMANDS = {
    key: COMMAND_LABELS[key]
    for key in COMMAND_LABELS
    if "italic" not in COMMAND_LABELS[key].lower()
    and "underline" in COMMAND_LABELS[key].lower()
}

PLAIN_TEXT_COMMANDS = {
    key: COMMAND_LABELS[key]
    for key in COMMAND_LABELS
    if "plain" in COMMAND_LABELS[key].lower()
}
...
def _build_writer_pac_codes():
    ...
    """
    _pac_pattern = _re.compile(r"row (\d+), column (\d+), with (.+)\.")
